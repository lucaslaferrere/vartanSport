...
export default function DashboardDueno() {
  ...
  const calcularDashboardData = useCallback((ventasData: IVenta[], _productosCount: number, gastosTotal: number) => {
    const hoy = new Date();
    ...
    const ingresosMes = ventasMes.reduce((sum, v) => sum + (v.total_final || v.precio_venta || v.total), 0);
    ...
    <Box>
      ...
      <Box sx={{ mb: { xs: 2, sm: 4 } }}>
        ...
        <Typography variant="body2" sx={{ color: colors.textSecondary, fontSize: { xs: '12px', sm: '14px' } }}>
          Vista general del negocio • {new Date().toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' })}
        </Typography>
      </Box>
      ...
      <Grid container spacing={{ xs: 2, sm: 2 }} sx={{ mb: { xs: 3, sm: 4 } }}>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard title="Ventas Hoy" value={stats.ventasHoy} icon="fa-solid fa-cart-shopping" trend={{ value: 12.5, isPositive: true }} />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard title="Ingresos del Mes" value={formatCurrency(stats.ingresosMes)} icon="fa-solid fa-dollar-sign" trend={{ value: 8.3, isPositive: true }} />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard title="Ganancia Real" value={formatCurrency(stats.gananciaReal)} icon="fa-solid fa-chart-line" subtitle={`Gastos: ${formatCurrency(stats.gastosMes)}`} />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard title="Ticket Promedio" value={formatCurrency(stats.ticketPromedio)} icon="fa-solid fa-receipt" trend={{ value: 5.2, isPositive: true }} />
        </Grid>
      </Grid>
      ...
      <Grid container spacing={{ xs: 2, sm: 2 }} sx={{ mb: { xs: 3, sm: 4 } }}>
        <Grid size={{ xs: 6, sm: 3 }}>
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 2 }, borderRadius: '12px', border: '1px solid #E5E7EB', textAlign: 'center' }}>
            ...
            <Typography sx={{ fontSize: { xs: '9px', sm: '10px' }, color: '#9CA3AF', mt: 0.25 }}>Este mes</Typography>
          </Paper>
        </Grid>
        <Grid size={{ xs: 6, sm: 3 }}>
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 2 }, borderRadius: '12px', border: '1px solid #E5E7EB', textAlign: 'center' }}>
            ...
            <Typography sx={{ fontSize: { xs: '9px', sm: '10px' }, color: '#9CA3AF', mt: 0.25 }}>Ventas/Visitas</Typography>
          </Paper>
        </Grid>
        <Grid size={{ xs: 6, sm: 3 }}>
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 2 }, borderRadius: '12px', border: '1px solid #E5E7EB', textAlign: 'center' }}>
            ...
            <Typography sx={{ fontSize: { xs: '9px', sm: '10px' }, color: '#9CA3AF', mt: 0.25 }}>Sin gastos</Typography>
          </Paper>
        </Grid>
        <Grid size={{ xs: 6, sm: 3 }}>
          <Paper elevation={0} sx={{ p: { xs: 1.5, sm: 2 }, borderRadius: '12px', border: '1px solid #E5E7EB', textAlign: 'center' }}>
            ...
            <Typography sx={{ fontSize: { xs: '9px', sm: '10px' }, color: '#9CA3AF', mt: 0.25 }}>Operativos</Typography>
          </Paper>
        </Grid>
      </Grid>
      ...
      <Grid container spacing={{ xs: 2, sm: 3 }}>
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 4 }}>
          <Paper elevation={0} sx={{ p: { xs: 2, sm: 3 }, borderRadius: '12px', border: '1px solid #E5E7EB', height: '100%' }}>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={metodosPago} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => {
            ...
            <Stack spacing={2}>
              <Box>
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.5 }}>
                  <Typography sx={{ fontSize: '13px', color: '#6B7280', fontWeight: 500 }}>Margen de Ganancia</Typography>
                ...
                <Box sx={{ width: '100%', height: 8, bgcolor: '#E5E7EB', borderRadius: 1, overflow: 'hidden' }}>
                  <Box sx={{ width: `${stats.ingresosMes > 0 ? (stats.gananciaReal / stats.ingresosMes) * 100 : 0}%`, height: '100%', bgcolor: '#059669' }} />
                </Box>
              </Box>
              ...
              <Box sx={{ borderTop: '1px solid #E5E7EB', pt: 2 }}>
                <Stack direction="row" justifyContent="space-between" sx={{ mb: 1 }}>
                  ...
                  <Typography sx={{ fontSize: '13px', fontWeight: 600 }}>{formatCurrency(stats.ingresosMes)}</Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between" sx={{ mb: 1 }}>
                  <Typography sx={{ fontSize: '12px', color: '#6B7280' }}>Ganancia Bruta</Typography>
                  <Typography sx={{ fontSize: '13px', fontWeight: 600, color: '#588a9e' }}>{formatCurrency(stats.gananciasBrutas)}</Typography>
                </Stack>
                <Stack direction="row" justifyContent="space-between" sx={{ mb: 1 }}>
                  <Typography sx={{ fontSize: '12px', color: '#DC2626' }}>Gastos Operativos</Typography>
                  <Typography sx={{ fontSize: '13px', fontWeight: 600, color: '#DC2626' }}>- {formatCurrency(stats.gastosMes)}</Typography>
                </Stack>
                <Box sx={{ borderTop: '1px solid #E5E7EB', pt: 1, mt: 1 }}>
                  <Stack direction="row" justifyContent="space-between">
                    <Typography sx={{ fontSize: '13px', fontWeight: 700 }}>Ganancia Neta</Typography>
              ...
              <Box sx={{ borderTop: '1px solid #E5E7EB', pt: 2 }}>
                <Typography sx={{ fontSize: '12px', color: '#6B7280', mb: 1, fontWeight: 500 }}>Indicadores Clave</Typography>
