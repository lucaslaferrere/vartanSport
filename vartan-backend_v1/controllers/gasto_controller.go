// controllers/gasto_controller.go
package controllers
...
func ObtenerResumenGastos(c *gin.Context) {
	...
	query.Select("categoria, SUM(monto) as total, COUNT(*) as cantidad").
		Group("categoria").
		Order("total DESC").
		Scan(&resumenCategoria)
	
	// Total general
	var totalGeneral float64
	var cantidadTotal int64
	query.Select("SUM(monto) as total, COUNT(*) as cantidad").
		Row().
		Scan(&totalGeneral, &cantidadTotal)
