...
func buildComprobantesQuery(c *gin.Context) (*gorm.DB, error) {
	...
	query := config.DB.Model(&models.Venta{}).
		...
		Where(`comprobante_url NOT IN (
			SELECT pago_venta.comprobante_url
			FROM pago_venta
			WHERE pago_venta.venta_id = venta.id
			  AND pago_venta.comprobante_url IS NOT NULL
			  AND pago_venta.comprobante_url <> ''
		)`)
